<template>
  <div class="container mx-auto max-w-7xl">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 gap-5 mb-6 auto-rows-fr">
      <div v-for="card in paginatedCards" :key="card.nm_id"
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex flex-wrap gap-1 mt-1">
            <Tag severity="secondary" :value="'nm: ' + card.nm_id" class="text-xs" />
            <Tag severity="secondary" :value="'Арт: ' + card.vendor_code" class="text-xs" />
            <Tag severity="secondary" :value="card.brand" class="text-xs" />
          </div>
          <div class="flex gap-1 mt-2 flex-wrap">
            <Tag v-for="size in card.sizes?.slice(0, 5)" :key="size.sku" severity="info" :value="size.tech_size || 'б/р'"
              class="text-xs" />
            <span v-if="card.sizes?.length > 5" class="text-xs text-gray-400">+{{ card.sizes.length - 5 }}</span>
          </div>
          <div class="text-xs text-gray-400 mt-2">Обновлено: {{ formatDate(card.updated_at) }}</div>
        </div>
      </div>
    </div>
...
const statsData = computed(() => {
  ...
  const updatedLastHour = filtered.filter(c => c.updated_at && (new Date() - new Date(c.updated_at)) < 3600000).length;
  return [
...
const paginatedCards = computed(() => filteredCards.value.slice((currentPage.value - 1) * pageSize, currentPage.value * pageSize));
...
onMounted(async () => {
  try { const res = await wbApi.getCards({ limit: 1000 }); cards.value = res.data.data; } catch (e) { console.error(e); }
});
</script>