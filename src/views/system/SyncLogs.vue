<template>
  <div class="container mx-auto">
    ...
    <DataTable :columns="columns" :data="paginatedLogs">
      ...
      <template #sync_at="{ item }">{{ formatDateTime(item.sync_at) }}</template>
      <template #records_count="{ item }">{{ formatNumber(item.records_count) }}</template>
...
const paginatedLogs = computed(() => filteredLogs.value.slice((currentPage.value - 1) * pageSize, currentPage.value * pageSize));
